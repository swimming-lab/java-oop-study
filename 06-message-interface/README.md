...
명령과 쿼리를 뒤섞으면 실행 결과를 예측하기가 어려워진다. isSatisfied 메서드처럼 겉으로 보기에는 쿼리처럼 보이지만 내부적으로 부수효과를 가질 경우 이해하기 어렵고(부수효과를 가질 것이라고 예상하지 못할 것이다.), 잘못 사용하기 쉬우며, 버그를 양산할 수 있다. 가장 깔끔한 해결책은 명령과 쿼리를 명확하게 분리하는 것이다.
...
- 참조 투명성
    - 수학적으로 어떤 함수 f(n)이 존재할 때 n값으로 1을 대입하면 그 결과가 3이면, f(1) = 3이다.
    - 수학에서 함수는 동일한 입력에 대해 항상 동일한 값을 반환하기 때문에 수학의 함수는 참조 투명성을 만족시킨다.
    - 어떤 값을 변하지 않는 성질을 불변성이라고 한다.
...
최근 함수형 프로그래밍이 인기가 있는데 이것은 부수효과가 존재하지 않는 수학적인 함수에 기반한다. 즉 참조 투명성의 장점을 극대화할 수 있는 프로그래밍 패러다임이다.

프로그램의 실행 결과를 이해하고 예측하기 쉽기 때문에 병렬 처리가 중요해진 최근에는 객체지향 언어들이 함수형 프로그래밍 패러다임을 접목시키고 있는 추세이다.